...
### Digitise uploaded PDF survey forms into a fillable fitter form — 🚧 extraction, office review, and fitter step-through form built 2026-09-06/08; completion email + voice input pending
...
- **Not yet built**: the completion → office-notification email, and voice input via a mic icon per field (browser Web Speech API for v1, designed to be easily swappable for Azure AI Speech later if reliability on fitters' actual phones — particularly iOS — turns out to need it). The office also wants the completion email to include a total trims-needed count — since Trims Needed is merged into the freeform notes field, this will need a small separate numeric "trims needed" field added per item alongside the notes so it can actually be totalled, not parsed out of prose.
