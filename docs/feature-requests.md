...
### Digitise uploaded PDF survey forms into a fillable fitter form — 🚧 extraction + office review built 2026-09-06, fitter form pending
...
- **Not yet built**: the fitter-facing step-through form (the biggest remaining piece — 30 items × 15 fields, dropdowns matching each field's fixed choice list, one combined Trims/Comments notes field per the office's own simplification, auto-save, progress indicator), voice input via a mic icon per field (browser Web Speech API for v1, designed to be easily swappable for Azure AI Speech later if reliability on fitters' actual phones — particularly iOS — turns out to need it), and the completion → office-notification email. Rough estimate given: ~1.5–2 more sessions.
